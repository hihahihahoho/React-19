...
function TestZoom({ onZoomChange, ref }: TestZoomProps = {}) {
  ...
  useImperativeHandle(ref, () => ({
    resetZoom: () => {
      // Reset scale to 1 with animation
      animate(scale, 1, { duration: 0.2 })
      // Reset position to center
      animate(x, 0, { duration: 0.2 })
      animate(y, 0, { duration: 0.2 })

      // Trigger zoom change callback if provided
      if (onZoomChange) {
        onZoomChange(false)
      }
    },
  ...
  useEffect(() => {
    const unsubscribe = scale.on("change", (newScale) => {
      if (onZoomChange) {
        if (newScale > 1) {
          onZoomChange(true)
        } else {
  ...
  const updatePosition = useCallback(
    (
      ...
      if (!refImg.current || !containerRef.current)
      ...
      const naturalWidth = refImg.current.naturalWidth
      const naturalHeight = refImg.current.naturalHeight
  ...
  const handleTouchMove = useCallback(
    (e: TouchEvent) => {
      ...
      // Only handle pinch gestures (2 fingers)
        ...
        if (refImg.current) {
          const imgRef = refImg.current.getBoundingClientRect()
  ...
  const handleMouseDown = useCallback((e: MouseEvent) => {
    ...
    if (refImg.current) {
      refImg.current.style.cursor = "grabbing"
    }
  }, [])
  const handleMouseUp = useCallback(
    (e: MouseEvent) => {
      if (refImg.current) {
        refImg.current.style.cursor = "grab"
      }
  ...
  return (
    <div>
      <div
        className="container-main flex h-[70svh] w-full items-center justify-center overflow-hidden rounded-lg bg-white shadow-lg"
        ref={containerRef}
      >
        <motion.img
          className="max-h-full max-w-full cursor-grab touch-none select-none rounded-lg object-contain object-center"
