...
export function fuzzySearch(searchText: string, content: string): number {
  ...
  for (let i = 0; i < contentLower.length && searchIndex < searchLower.length; i++) {
    if (contentLower[i] === searchLower[searchIndex]) {
      ...
      if (i === 0 || contentLower[i - 1] === ' ' || contentLower[i - 1] === '-') {
        score += 0.5;
      }
    } else {
...
export function fuzzyFilterStrings(
  ...
  const results = searchableStrings.map((searchStr, index) => {
    ...
    if (bestScore < 0.5) {
      ...
      for (const filterVal of filterValues) {
        const fuzzyScoreTransliterated = fuzzySearch(filterVal, transliteratedText);
        if (fuzzyScoreTransliterated > bestScore) bestScore = fuzzyScoreTransliterated;
      }
    }
